function initengahan(){
  ...
  async function menuju(){await swals.fire('Sẵn sàng chưa!!', 'Giờ cùng xem bất ngờ của anh dành cho bé nhaa 💗 ', 'success');window.location = "https://sharengay.com/love/index1.html";}
  function tombol() {
    wallpaper.style = "transform: scale(1);";
...
async function menuju() {
  await swals.fire({
    title: 'Sẵn sàng chưa!!',
    text: 'Giờ cùng xem bất ngờ của anh dành cho bé nhaa 💗',
    icon: 'success',
    confirmButtonText: 'OK'
  }).then((result) => {
    if (result.isConfirmed) {
      // Play the audio when 'OK' is clicked
      document.getElementById("linkmp31").play();

      // Show the carousel overlay
      document.getElementById("bannerCarousel").style.display = "flex";
    }
  });
}

// Function to close the banner carousel
function closeBannerCarousel() {
  document.getElementById("bannerCarousel").style.display = "none";
  // Pause the audio when the carousel is closed
  document.getElementById("linkmp3").pause();
}
